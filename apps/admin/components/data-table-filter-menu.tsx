...
import { DataTableRangeFilter } from "@t2p-admin/ui/components/data-table/data-table-range-filter";
...
import {
  ...
  CommandItem,
  CommandList,
} from "@t2p-admin/ui/components/command";
...
import { useDebouncedCallback } from "@t2p-admin/ui/hooks/use-debounced-callback";
import {
  getDefaultFilterOperator,
  getFilterOperators,
} from "@t2p-admin/ui/lib/data-table";
import { formatDate } from "@t2p-admin/ui/lib/format";
import { generateId } from "@t2p-admin/ui/lib/id";
import { getFiltersStateParser } from "@t2p-admin/ui/lib/parsers";
import { cn } from "@t2p-admin/ui/lib/utils";
import type {
  ExtendedColumnFilter,
  FilterOperator,
} from "@t2p-admin/ui/types/data-table";
      ...
      >
        ...
        <Select
          ...
          onValueChange={(value: FilterOperator) =>
            onFilterUpdate(filter.filterId, {
              operator: value,
              value: value === "equals" || value === "not" ? "" : filter.value,
            })
          }
        >
          <SelectTrigger
...
function onFilterInputRender<TData>({
  ...
  if (filter.operator === "equals" || filter.operator === "not") {
    return (
      <div
        id={inputId}
        role="status"
        aria-label={`${column.columnDef.meta?.label} filter is ${
          filter.operator === "equals" ? "equals" : "not equals"
        }`}
        aria-live="polite"
  ...
  switch (filter.variant) {
    ...
    case "range": {
      if (
        (filter.variant === "range" && filter.operator === "between") ||
